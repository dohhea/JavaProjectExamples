...
public class OthelloClient {
    ...
    private void go() {
       // build GUI
        ...
        String[] list = {OthelloMessage.ALL};
   ...
   private class LogButtonListener implements ActionListener {
      ...
      private void processLogin() {
    	  ...
    	  try {
       		  writer.writeObject(new OthelloMessage(OthelloMessage.MsgType.LOGIN, user, "", ""));
              writer.flush();
      ...
      private void processLogout() {
   	   	  ...
   	   	  if (choice == JOptionPane.YES_OPTION) {
   	   		  try {
   	             writer.writeObject(new OthelloMessage(OthelloMessage.MsgType.LOGOUT, user, "", ""));
   	             writer.flush();
   ...
   public class SendButtonListener implements ActionListener {
	   public void actionPerformed(ActionEvent ev) {
	   	  ...
	   	  try {
	   		  ...
	   		  writer.writeObject(new OthelloMessage(OthelloMessage.MsgType.CLIENT_MSG, user, to, outgoing.getText()));
	   		  writer.flush();
	   		  outgoing.setText("");
   ...
   public class IncomingReader implements Runnable {
     public void run() {
    	 ...
    	 try {
    		 while (true) {
        		 message = (OthelloMessage) reader.readObject();     	 // 서버로기 부터의 메시지 대기                   
               	 type = message.getType();
               	 if (type == OthelloMessage.MsgType.LOGIN_FAILURE) {	 // 로그인이 실패한 경우라면
               		 JOptionPane.showMessageDialog(null, "Login이 실패하였습니다. 다시 로그인하세요");
    	             frame.setTitle(frameTitle + " : 로그인 하세요");
    	             logButton.setText("Login");
               	 } else if (type == OthelloMessage.MsgType.SERVER_MSG) { // 메시지를 받았다면 보여줌
               		 if (message.getSender().equals(user)) continue;  // 내가 보낸 편지면 보일 필요 없음
               		 incoming.append(message.getSender() + " : " + message.getContents() + "\n");
               		 qScroller.getVerticalScrollBar().setValue(qScroller.getVerticalScrollBar().getMaximum());
               		 game.backGroundMusic("채팅.wav");
               	 } else if (type == OthelloMessage.MsgType.LOGIN_LIST) {
               		 ...
               		 users[0] =  OthelloMessage.ALL;	// 리스트 맨 앞에 "전체"가 들어가도록 함
               		 counterParts.setListData(users);
               		 counterParts.setSelectedIndex(0);
               		 frame.repaint();
               		 game.backGroundMusic("로그인.wav");
