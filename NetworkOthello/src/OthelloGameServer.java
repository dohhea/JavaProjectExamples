...
public class OthelloGameServer {
    ...
    private class ClientHandler implements Runnable {
    	...
    	public void run() {
    		OthelloMessage message;
    		OthelloMessage.MsgType type;
    		try {
    			while (true) {
    				
    				// 읽은 메시지의 종류에 따라 각각 할일이 정해져 있음
           	 		message = (OthelloMessage) reader.readObject();	  // 클라이언트의 전송 메시지 받음
           	 		type = message.getType();
           	 		if (type == OthelloMessage.MsgType.LOGIN) {		  // 클라이언트 로그인 요청
           	 			handleLogin(message.getSender(),writer);	  // 클아이언트 이름과 그에게 메시지를
           	 														  // 보낼 스트림을 등록
           	 		}
           	 		else if (type == OthelloMessage.MsgType.LOGOUT) {	  // 클라이언트 로그아웃 요청
           	 			handleLogout(message.getSender());			  // 등록된 이름 및 이와 연결된 스트림 삭제
           	 			writer.close(); reader.close(); sock.close(); // 이 클라이언트와 관련된 스트림들 닫기
           	 			return;										  // 스레드 종료
           	 		}
           	 		else if (type == OthelloMessage.MsgType.CLIENT_MSG) {
           	 			handleMessage(message.getSender(), message.getReceiver(), message.getContents());
           	 		}
           	 		else if (type == OthelloMessage.MsgType.NO_ACT) {
           	 			//  무시해도 되는 메시지
           	 			continue;
           	 		}
           	 		else if (type == OthelloMessage.MsgType.GAME_INFO){
           	 			broadcastMessage(new OthelloMessage(OthelloMessage.MsgType.GAME_INFO,message.i,message.j));
	           	  
           	 		}
           	 		else if (type == OthelloMessage.MsgType.GAME_START){
           	 			broadcastMessage(new OthelloMessage(OthelloMessage.MsgType.GAME_START, message.userCheck));
    ...
    private synchronized void handleLogin(String user, ObjectOutputStream writer) {
	   try {
		   ...
		   if (clientOutputStreams.containsKey(user)) {
			   writer.writeObject(
				   new OthelloMessage(OthelloMessage.MsgType.LOGIN_FAILURE, "", "", "사용자 이미 있음"));
			   return;
		   }
	   } catch (Exception ex) {
	   ...
	   broadcastMessage(new OthelloMessage(OthelloMessage.MsgType.LOGIN_LIST, "", "", makeClientList()));
    }  // close handleLogin
    ...
    private synchronized void handleMessage(String sender, String receiver, String contents) {
	   ...
	   if (receiver.equals(OthelloMessage.ALL)) {			// "전체"에게 보내는 메시지이면
		   broadcastMessage(new OthelloMessage(OthelloMessage.MsgType.SERVER_MSG, sender, "", contents));
		   return;
	   }
	   // 특정 상대에게 보내는 경우라면
	   ObjectOutputStream write = clientOutputStreams.get(receiver);
	   try {
		   write.writeObject(new OthelloMessage(OthelloMessage.MsgType.SERVER_MSG, sender, "", contents));
	   } catch (Exception ex) {
		   System.out.println("S : 서버에서 송신 중 이상 발생");
		   ex.printStackTrace();
	   }
    ...
    private void broadcastMessage(OthelloMessage message) {
